    ...
    $result = mysqli_query($con, $sql);
		...
		<table style="border: 10px solid black; background: white;">
			<tr> <!-- wiersz z nazwami kolumn -->
				<th style="border: 5px solid green;">ID</th>
				<th style="border: 5px solid green;">TYTUŁ</th>
				<th style="border: 5px solid green;">OPIS</th>
				<th style="border: 5px solid green;">UTWORZENIE.</th>
				<th style="border: 5px solid green;">MODYFIKACJA</th>
				<th style="border: 5px solid green;">WYGAŚNIĘCIE</th>
				<th style="border: 5px solid green;">CENA NETTO</th>
				<th style="border: 5px solid green;">VAT</th>
				<th style="border: 5px solid green;">DOSTĘPNYCH SZTUK</th>
				<th style="border: 5px solid green;">STATUS SZTUK</th>
				<th style="border: 5px solid green;">KATEGORIA</th>
				<th style="border: 5px solid green;">GABARYT</th>
				<th style="border: 5px solid green;">OPCJA</th>
			</tr>
		<?php // wyświetlanie wierszy z danymi elementami
		while ($row = mysqli_fetch_array($result)) {
			...
			<tr>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["id"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["tytul"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["opis"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["data_utworzenia"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["data_modyfikacji"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["data_wygasniecia"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["cena_netto"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["podatek_vat"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["ilosc_dostepnych_sztuk_w_magazynie"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["status_dostepnosci"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["kategoria"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;"><?php echo $row["gabaryt_produktu"]; ?></td>
				<td style="border: 5px solid lime; font-size: 23px;">
					<a href="?action=EdytujProdukt&id=<?php echo $row["id"]; ?>">Edytuj</a>
					<a href="?action=UsunProdukt&id=<?php echo $row["id"]; ?>" onclick="return confirm('Czy na pewno chcesz usunąć ten rekord?')">Usuń</a>
				</td>
			</tr>
			<?php
		}
		?>
			<tr>
				<td colspan="13" style="border: 5px solid lime; font-size: 23px; text-align: center;">
					<a href="?action=DodajProdukt">Dodaj produktę</a>
				</td>
			</tr>
		</table>
		<?php
...
if (isset($_GET['action']) && $_GET['action'] == 'DodajProdukt') {
	...
	'cena_netto' => '', 'podatek_vat' => '', 'ilosc_dostepnych_sztuk_w_magazynie' => '', 'status_dostepnosci' => '', 'kategoria' => '', 'gabaryt_produktu' => '');
    ...
    <form class="popup" method="post" action="adminsite.php">
        <table class="popup">  
			...
			<tr class="popup">	<!-- podatek vat -->
				...
				<td class="popupp"> <input class="popup" type="number" step="0.01" name="podatek_vat" value="<?php echo $row['podatek_vat']; ?>"></input> </td>
			</tr>
			...
			<tr class="popup">	<!-- kategoria -->
				...
				<td class="popupp"> <select class="kategorie" name="kategoria" id="kategorie"> <?php echo PobierzKategorie($conn); ?> </select>
            </tr>	
			<tr class="popup">	<!-- gabaryt produktu -->
                <td class="popupl"> <label class="popup" for="gabaryt_produktu">Gabaryt: </label><br> </td>
				<td class="popupp"> <input class="popup" type="text" name="gabaryt_produktu"value="<?php echo $row['tytul']; ?>"> </input>  </td>
            </tr>
			<tr class="popup">	<!-- przyciski -->
                <td class="popupl" > <input class="popup" type="submit" name="<?php echo $czynnosc; ?>" value="Ok"> </td>
                <td class="popupp" > 
...
// obsługa formularza po edycji produktu / zapisanie danych po edycji
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['zapiszprodukt'])) {
	...
	data_modyfikacji = '$data_modyfikacji', data_wygasniecia = '$data_wygasniecia', cena_netto = '$cena_netto', podatek_vat = '$podatek_vat', ilosc_dostepnych_sztuk_w_magazynie = '$ilosc_dostepnych_sztuk_w_magazynie', 
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['dodajprodukt'])) {
	...
	ilosc_dostepnych_sztuk_w_magazynie, status_dostepnosci, kategoria, gabaryt_produktu) 
	...
	'$status_dostepnosci','$kategoria','$gabaryt_produktu') LIMIT 1";
