...
function PobierzProdukty()
{
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $options .= '<option value="' . $row['id'] . '">' . $row['tytul'] . '</option>';
        }
    }
...
<script>
function IntoSklep(){
    window.location.href = 'sklep.php';
}
</script>
    ...
    $result = mysqli_query($con, $sql);
		...
		<table class="product">
			<tr> <!-- wiersz z nazwami kolumn -->
				...
				<th class="product">D. UTW.</th>
				<th class="product">D. MOD.</th>
				<th class="product">D. WYG.</th>
				<th class="product">CENA NETTO</th>
				<th class="product">VAT</th>
				<th class="product">ILOŚĆ</th>
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			<tr>
				...
				<td class="product">
					<img class="product" src="data:image/jpeg;base64,<?php echo base64_encode($row["zdjecie"]); ?>">
				</td>
				<?php // różne pola w zależności od strony
				if (basename($_SERVER['PHP_SELF']) == 'adminsite.php'){
				?>
				<td class="product"> <!-- wariant dla adminsite -->
				...
				<td class="product">					
					<form method="post" action="">
						Dodaj do koszyka: <input type="hidden" name="id_produktu" value='<?php echo $row["id"]; ?>'></input><br>
			...
			<tr>
				<td colspan="14" class="product" style="text-align: center;">
					<a class="product" href="?action=DodajProdukt"> &nbsp Dodaj produkt &nbsp</a>
				</td>
			</tr>
		<?php } ?>
		</table>
    ...
    <form class="popup" method="post" action="adminsite.php" enctype="multipart/form-data">
        <table class="popup">  
			...
			<tr class="popup">	<!-- obrazek -->
				...
				<td class="popupp"> <input class="popup" type="file" name="zdjecie" id="dodanezdjecie">  </td>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['zapiszprodukt'])) {
    ...
    $zdjecie = 'img/BrakZdjęcia.jpg'; // wartość domyślna dla obrazka
    ...
    if ($_FILES['zdjecie']['error'] == UPLOAD_ERR_OK) {
        // odczytanie zawartości pliku
        $zdjecie_tmp = $_FILES['zdjecie']['tmp_name'];
        $zdjecie = mysqli_real_escape_string($conn, file_get_contents($zdjecie_tmp));
        $updateZdjecie = ", zdjecie = '$zdjecie'";
    }
    ...
    $sql = "UPDATE product_list SET tytul = '$tytul', opis = '$opis', data_utworzenia = '$data_utworzenia',
        ...
        kategoria = '$kategoria', gabaryt_produktu = '$gabaryt_produktu' $updateZdjecie WHERE id = '$id' LIMIT 1";
    ...
    if ($result) {
        ...
        echo 'coś poszło nie tak. Błąd SQL: ' . mysqli_error($conn);
    }
}
